...
from hendlers.states_man import (
    add_photo,
    incorrect_photo,
    add_age,
    add_about,
...
async def test_men_state_add_photo(storage, bot):
    ...
    assert await state.get_state() == StatesMenQuestionnaire.PHOTO
...
async def test_men_state_add_age(storage, bot):
    ...
    assert await state.get_state() == StatesMenQuestionnaire.AGE
    message.answer.assert_called_once_with(
        "Введите ваш возраст: "
    )
...
async def test_men_state_add_find_me(storage, bot):
    ...
    assert await state.get_state() == StatesMenQuestionnaire.FIND
...
async def test_men_state_check_status(storage, bot):
    ...
    assert await state.get_state() == StatesMenQuestionnaire.STATUS
    menu = await gen_replay_keyboard(['Только телеграм'])
    message.answer.assert_called_once_with("Здесь вы можете оставить свой"
                                           " никнейм в любой из соц сетей\n"
                                           "либо нажмите на кнопку ниже если"
                                           " хотите оставить только телеграм.",
                                           reply_markup=menu)
